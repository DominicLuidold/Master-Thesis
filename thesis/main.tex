...
Bevor die technische Umsetzung im Detail erläutert wird, werden im folgenden Abschnitt die umgesetzten Funktionalitäten anhand der in Abschnitt \ref{sec:priorisierung-anforderungen} ab Seite \pageref{sec:priorisierung-anforderungen} priorisierten Anforderungen dargelegt und im Zuge dessen in Bezug gestellt. Die entwickelten Lösungen decken dabei nicht immer die gesamte Anforderung ab, was im Zuge von Kapitel \ref{chap:möglichkeiten-zur-weiterentwicklung} sowie Kapitel \ref{chap:evaluation-prototyp} ab Seite \pageref{chap:möglichkeiten-zur-weiterentwicklung} beziehungsweise Seite \pageref{chap:evaluation-prototyp} stellenweise noch einmal aufgegriffen wird.
...
Die Umsetzung des Frontends der EthicsVision Plattform mithilfe von Angular entspricht einer \acl{spa}. Im Gegensatz zu einer klassischen \ac{mpa}, bei der der Server für die Präsentation zuständig ist und das fertige Resultat an den Browser übermittelt, ist die Präsentationsschicht sowie weitere Logik Teil des Webbrowsers, der die Applikation ausführt. Konkret bedeutet das, dass jede Navigation zu einer neuen Seite lokal im Browser stattfindet, indem Teile des \ac{ui} dynamisch ausgetauscht werden. Die sogenannten \textit{View}s werden dabei im \ac{dom} abhängig von ausgeführten Aktionen ersetzt und stellen neben statischen Inhalten jene Daten dar, die mittels Abfrage vom Server beziehungsweise dem Backend bereitgestellt wurden. \cite[5\psqq]{scott_spa_2015}

Abbildung \ref{fig:scott-spa-shell} auf Seite \pageref{fig:scott-spa-shell} zeigt die von Scott in \cite{scott_spa_2015} beschriebene \textit{Shell}, die der Hauptbestandteil einer \ac{spa} darstellt und  einer statischen HTML-Datei entspricht, die beim initialen Öffnen der Webapplikation vollständig heruntergeladen wird. Diese Datei enthält neben allgemeinen Strukturen und dem Styling vor allem ein Element\footnote{Im zitierten Beispiel handelt es sich um ein \texttt{div}-Element, im umgesetzten Prototyp hingegen um einen von Angular bereitgestelltes \texttt{app-root} Container"=Element.}, das den dynamisch eingesetzten Views übergeordnet ist. Der für Anwender:innen ohne sichtbaren Reload durchgeführte Austausch des Inhalt wird dabei über die angesprochene \ac{dom}"=Manipulation umgesetzt, welche ein Kernelement einer \ac{spa} darstellt und das Gefühl einer nativen Applikation vermittelt. \cite[8, 10\psqq]{scott_spa_2015}
...
\begin{itemize}
    ...
    \item \textbf{CSS Selector:} Der CSS-Selector wird verwendet, um eine Komponente an beliebiger Stelle in einem Template einzubinden und darzustellen. Der Selector kann wie ein HTML-Tag verwendet werden, beispielsweise \texttt{<app-selector>} .
...
Um den entwickelten Prototyp aktiv nutzen zu können, müssen folgende Schritte ausgeführt werden:
...
Der Ethikantrag der \acl{fek} der \acl{fhv} wird momentan beim Ausführen von \texttt{make setup} standardmäßig in die Datenbank eingespielt und ist in der Frontend"=Applikation an mehreren Stellen durch eine eindeutige ID als vorläufig fester Bestandteil fest vorgegeben. Das Abändern des Ethikantrages ist hingegen über das Bearbeiten der \texttt{FormFixtures} Klasse im Namespace \url{App\\Form\\DataFixtures\\Production} möglich.

\chapter{Möglichkeiten zur Weiterentwicklung des Prototyps}
\label{chap:möglichkeiten-zur-weiterentwicklung}
