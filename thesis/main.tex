...
Bei genauerer Betrachtung der technischen Gegebenheiten (welche im genannten Abschnitt in Bezug auf die \ac{dx} nur teilweise aufgearbeitet wurden) differenziert sich dieses Bild jedoch:
\begin{itemize}
    ...
    \textbf{Schlussfolgerung:} Es ist davon auszugehen, dass eine Aktualisierung des zugrundeliegenden Betriebssystems auf den aktuellsten Stand (Version \texttt{22.04 LTS} mit initialer Veröffentlichung im April 2022) mit erheblichem Aufwand verbunden ist und bei zahlreichen Abhängigkeiten der Applikation zu Komplikationen führen wird.
    ...
    \textbf{Schlussfolgerung:} Die erforderliche Einarbeitung in das Projekt, um die notwendigen Anpassungen anhand der Anforderungen vornehmen, nicht benötigte Funktionalitäten evaluieren und entfernen sowie neue Möglichkeiten hinzufügen zu können, würde den Umfang dieser Masterarbeit deutlich überschreiten.
    \item Das \ac{ecs} ist auf eine Nutzung von 20 bis hin zu mehreren Hundert Anträgen pro Monat ausgelegt. Der benötigte Speicherplatz bei 20 Anträgen pro Monat wird pro zehn Jahre auf zirka 92 GB geschätzt, während die Entwicklungs-Instanz bereits mindestens 10 GB Speicherplatz benötigt. \cite{medizinische_universitat_wien_ecs-handbook_requirements-2021, ethics_commission_system_organization_ecs_2021}\\
    \textbf{Schlussfolgerung:} Der genannte Speicherbedarf entspricht im produktiven Einsatz zwar keinem außergewöhnlich hohem Verbrauch, gerade für eine Entwicklungsumgebung spielt der Speicherplatz und damit auch der \ac{dx} jedoch eine wichtige Rolle.
\end{itemize}

Unter Berücksichtigung der technischen Analyse in Abschnitt \ref{sub-sec:ecs} ab Seite \pageref{sub-sec:ecs} sowie der bisher genannten Punkte wird deutlich, dass das \acl{ecs} eine ausgereifte und umfangreiche Software"=Lösung für das Erstellen und Bearbeiten von Ethikanträgen darstellt. Vor allem aufgrund der zeitlichen Begrenzungen, die mit der Entwicklung eines initialen Prototyps für die in dieser Arbeit definierten Anforderungen einhergehen, kann das \ac{ecs} jedoch nicht als Basis für die Umsetzung der angedachten Funktionen herangezogen werden.
...
Die Frameworks und Technologien wurden zum einen aufgrund bestehender Vorkenntnisse gewählt, zum anderen jedoch vor allem mit dem Hintergedanken, dass damit sowohl das \ac{ui} als auch die \enquote{im Hintergrund} liegende Logik jederzeit unabhängig voneinander ausgetauscht werden können, sollte dies notwendig sein. Genauere technische Erläuterungen und die Erhebung langfristiger Implikationen dieser Entscheidung finden sich in Abschnitt \ref{sec:ausarbeitung-technische-umsetzung} ab Seite \pageref{sec:ausarbeitung-technische-umsetzung}.
...
\noindent\underline{Abbildungen:} Abschnitt \ref{appendix:ethics-vision-formular} ab Seite \pageref{appendix:ethics-vision-formular} enthält im Zusammenhang mit den ausgearbeiteten Funktionalitäten für Anforderungen \hyperref[sub-sub-sec:abgeleitete-anforderungen-vorfeld-antrag]{\#2} und \hyperref[sub-sub-sec:abgeleitete-anforderungen-während-erstellung-einreichung]{\#5} und \hyperref[sub-sub-sec:abgeleitete-anforderungen-nach-einreichung]{\#8} Abbildungen zum Aussehen und der Funktionsweise des dem Ethikantrag zugrundeliegenden Formulares sowie der automatisierten Validierung bereitgestellter Daten.
...
\textbf{Anmerkung:} Der dargestellte Ethikantrag wurde mit den Eingaben \texttt{F\&E-Arbeit von Studierenden} zur Frage \enquote{In welchem Rahemn wird das Forschungsvorhaben bzw. -projekt umgesetzt und finanzeirt?}, \texttt{Ja} zur Frage \enquote{Wird eine Studie an oder mit Menschen durchgeführt?} und \texttt{Ja} zur Frage \enquote{Wird ein Prototyp entwickelt?} generiert.
